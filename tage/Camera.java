...
public class Camera
{
	...
	/** orients this Camera so that it turns on the Y-axis */
	public void yaw(float amount) {
		yaw += amount;

		Matrix4f rotationMatrix = new Matrix4f().rotateY(amount);

		rotationMatrix.transformDirection(u);
		rotationMatrix.transformDirection(n);

		// Recalculate to maintain orthogonality
		v.set(n).cross(u).normalize();
	}
	/** orients this Camera so that it turns on the X-axis */
	public void pitch(float amount) {
		pitch += amount;

		Matrix4f rotationMatrix = new Matrix4f().rotate(amount, u.x, u.y, u.z);

		rotationMatrix.transformDirection(v);
		rotationMatrix.transformDirection(n);
