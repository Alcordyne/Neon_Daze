...
public class GameObject
{
	...
	*/
	...
	/** Rotates object around the Y axis, updates matrix with new yaw value */
	public void yaw(float amount) {
		// Rotation around Y-axis
		yaw += amount;

		// Update rotation matrix with the new yaw value
		localRotation.identity();
		localRotation.rotateY(yaw);
		update(); // This will apply the new rotation to the world transform
	}

	public void globalYaw(float angle) {
		Matrix4f rotationMatrix = new Matrix4f().rotationY(angle);
		this.setLocalRotation(rotationMatrix.mul(this.getLocalRotation()));
	}
	/** Rotates object around the X axis, updates matrix with new pitch value */
